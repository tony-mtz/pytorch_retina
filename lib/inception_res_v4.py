...
def get_inception_res_v4(patch_height, patch_width, n_ch):
    ...
    reduction_block_2 = reduction2(inception_block_2)
    print(reduction_block_2.shape)
    
    inception_block_3 = inception3(reduction_block_2)
    print(inception_block_3.shape)
    
    '''
    up1 = UpSampling2D(size=(2, 2), data_format='channels_last')(stem_segment)
    up2 = UpSampling2D(size=(4, 4), data_format='channels_last')(inception_block_1)
    up3 = UpSampling2D(size=(8, 8), data_format='channels_last')(inception_block_2)
    up4 = UpSampling2D(size=(16, 16), data_format='channels_last')(inception_block_3)
    
    dil_block_1 = dil_block(up1)
    dil_block_2 = dil_block(up2)
    dil_block_3 = dil_block(up3)
    block_4 = Conv2D(2, (3, 3), activation='relu', padding='same', data_format='channels_last')(up4)
    '''
    
    dil_block_1 = dil_block(stem_segment)
    dil_block_2 = dil_block(inception_block_1)
    dil_block_3 = dil_block(inception_block_2)
    
    up1 = UpSampling2D(size=(2, 2), data_format='channels_last')(dil_block_1)
    up2 = UpSampling2D(size=(4, 4), data_format='channels_last')(dil_block_2)
    up3 = UpSampling2D(size=(8, 8), data_format='channels_last')(dil_block_3)
    
    up4 = UpSampling2D(size=(16, 16), data_format='channels_last')(inception_block_3)
    block_4 = Conv2D(2, (3, 3), activation='relu', padding='same', data_format='channels_last')(up4)
    
    print(dil_block_1.shape)
    print(dil_block_2.shape)
    print(dil_block_3.shape)
    print(block_4.shape)
    merge = Add()([up1, up2, up3, block_4])
    
    conv1 = Conv2D(2, (3, 3), activation='relu', padding='same', data_format='channels_last')(merge)
    conv2 = Conv2D(2, (3, 3), activation='relu', padding='same', data_format='channels_last')(conv1)
    final = core.Reshape((patch_height*patch_width, 2))(conv2)
    #merge = core.Permute((2,1))(merge)
